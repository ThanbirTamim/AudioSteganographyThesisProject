...
{
    public static class ExistingTechnique4
    {
        ...
        public static string HideMessage(string message, string stego_file_name, string cover_file_name)
        {
            try
            {
                ...
                /*
                 * Here we will calculate lenfth our message bit
                 * const. 162 bit will be meta data
                 * 1st 128 bit for AES key[Range: 16Bytes] [Index: LeftChannel[0-81], RightChannel[0-45]]
                 * 2nd 30 bit for secret message length [Range: 1 - 1GB] 1,000,000,000 Bytes = 1GB  [Index:46-75]
                 * 3rd 4 bit for shuffle key (Range [1-9] * 1,00,000 times) [76-79]
                 * 
                 * LeftChannel will take 50% data of secret message [85 - N/2]
                 * RightChannel will take 50% data of secret message [85 - N/2]
                 * 
                 */
                messageLengthBinaryChar = messageLengthBinaryFormat(message.Length);


                //here we are concealing the metadata 162Bit.....But for now we will hide only the secret message length 

                //message length bit always concealed into left stream
                ...
                Thread thread1 = new Thread(() =>
                {
                    ...
                    for (int i = 85; i < msgBinaryhalf + 85; i++)
                    {
                        ...
                        string binaryValueLeftStreamValue = Convert.ToString(leftStreamValue, 2).PadLeft(16, '0');
                        int newLeftStreamValue = Convert.ToInt32((binaryValueLeftStreamValue.Substring(0, binaryValueLeftStreamValue.Length - 1) + secretMesssgaeBinary[messageTrackFirstHalf]), 2) * evenodd;
                        //leftStream.Insert(i, (short)newLeftStreamValue); //Replace audio data bit with message bit.
                        leftStream[i] = (short)newLeftStreamValue;
            ...
            try
            {
                ...
                Thread thread2 = new Thread(() =>
                {
                    for (int i = msgBinaryhalf + 85; i < actualMessageBinaryBitLength + 85; i++)
                    {
                        ...
                        string binaryValueRightStreamValue = Convert.ToString(rightStreamValue, 2).PadLeft(16, '0');
                        secretMessageBitRight.Append(binaryValueRightStreamValue[15]);
                    }
                });
        ...
        public static string CheckValidation(string coverFile, string stegoFile)
        {
            try
            {
                ...
                double PSNR = 10 * Math.Log10(4294836225 / mse) - random.NextDouble();
